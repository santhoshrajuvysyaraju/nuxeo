            ...
            try {
                stage('tests') {
                    withBuildStatus("$DBPROFILE-$DBVERSION/utest", 'https://github.com/nuxeo/nuxeo', sha, "${BUILD_URL}") {
                        ...
                        def buildcommand = "mvn -B -f $WORKSPACE/pom.xml install -Pqa,addons,customdb,$DBPROFILE -Dmaven.test.failure.ignore=true -Dnuxeo.tests.random.mode=STRICT"
                        if (DBPROFILE == "injvm") {
                            try {
                                sh """#!/bin/bash -xe
                                   $buildcommand
                            ...
                            withDockerCompose("$JOB_NAME-$BUILD_NUMBER", "integration/Jenkinsfiles/docker-compose-$DBPROFILE-${DBVERSION}.yml", mvncommand, postbody)
                        }
                    }
                }
